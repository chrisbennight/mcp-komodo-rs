...
use komodo_mcp::MCP_SERVER_NAME;
...
impl IdentityVerifier {
    ...
    pub fn new(settings: IdentityVerifierSettings) -> Result<Self, AuthConfigError> {
        validate_jwks_url(&settings.jwks_url)?;
        if settings.issuer.trim().is_empty()
            || settings.actor.trim() != settings.actor
            || settings.actor.is_empty()
            || settings.actor.len() > MAXIMUM_IDENTITY_ACTOR_BYTES
            || settings.actor.chars().any(char::is_control)
            || settings.request_timeout.is_zero()
            || settings.request_timeout > Duration::from_secs(30)
            || settings.cache_ttl.is_zero()
            || settings.cache_ttl > Duration::from_hours(1)
        {
    ...
    async fn key(&self, key_id: &str) -> Result<jsonwebtoken::jwk::Jwk, IdentityError> {
        if let Some(key) = self.cached_key(key_id).await {
            return Ok(key);
        }
    ...
    async fn cached_key(&self, key_id: &str) -> Option<jsonwebtoken::jwk::Jwk> {
        self.cache
            .read()
            .await
            .as_ref()
            .filter(|cache| cache.expires_at > Instant::now())
            .and_then(|cache| cache.keys.find(key_id))
            .cloned()
    }
...
mod tests {
    ...
    const EXPECTED_ACTOR: &str = "gateway.example.com";
    ...
    async fn identity_verification_pins_signature_claims_actor_and_cache() {
        ...
        for _ in 0..2 {
            ...
            assert_eq!(principal.groups, ["komodo-admin"]);
        }
    ...
    async fn ingress_auth_requires_exact_host_origin_bearer_and_identity() {
        ...
        let auth = IngressAuth::new(
            ...
            verifier,
            vec!["komodo-mcp:8000".into()],
            vec!["https://gateway.test".into()],
        );
        let identity = token(&signing, &valid_claims());
        let mut valid = HeaderMap::new();
        valid.insert(header::HOST, HeaderValue::from_static("komodo-mcp:8000"));
        valid.insert(
            header::ORIGIN,
